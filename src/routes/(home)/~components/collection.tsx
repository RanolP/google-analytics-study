...
import { createShownEventHandler } from '@/shared/hooks/createShownEventHandler';

interface Props {
  expose: (seasonId: string) => void;
...
export function Collection(props: Props) {
  ...
  const shownHandler = createShownEventHandler<{ id: string }>((entry, ctx) => {
        ...
        >
          {seasons()?.items.map((season) => (
            <li
              ...
              shrink-0
              w="var(--width)"
              ref={shownHandler.watch({ id: season.id })}
            >
